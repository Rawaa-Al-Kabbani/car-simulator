...
module.exports = function (width, length, x, y, direction, stepsOfMovement) {
  ...
  function setCorners(directionToCheck) {
    ...
    console.log(
      "StartXcar",
      carDimentions.getStartXcar(),
      "StartYcar",
      carDimentions.getStartYcar()
    );
    console.log(
      "EndXcar  ",
      carDimentions.getEndXcar(),
      "EndYcar  ",
      carDimentions.getEndYcar()
    );
  }
  ...
  console.log(
    "StartXroom",
    roomDimentions.getStartXroom(),
    "StartYroom",
    roomDimentions.getStartYroom()
  );
  console.log(
    "EndXroom  ",
    roomDimentions.getEndXroom(),
    "EndYroom  ",
    roomDimentions.getEndYroom()
  );

  setCorners(carDirection.getDirection());
  if (
    carDimentions.startXcar <= newStartXroom ||
    carDimentions.startXcar >= newEndXroom
  ) {
    console.log(
      `insert x where x is between ${roomDimentions.startXroom} and ${
        roomDimentions.endXroom - 1
      }`
    );
    return;
  }
  if (
    carDimentions.startYcar <= newStartYroom ||
    carDimentions.startYcar >= newEndYroom
  ) {
    console.log(
      `insert y where y is between ${roomDimentions.startYroom} and ${
        roomDimentions.endYroom - 1
      }`
    );
    return;
  } else {
    ...
    if (stepsOfMovement) {
      ...
      if (
        carDimentions.getStartXcar() < roomDimentions.getStartXroom() ||
